...
def parse_args():
    ...
    parser.add_argument(
        '--model', type=str, help='Model name to generate continuation. HuggingFace/OpenAI.', default="meta-llama/llama-2-7b-hf")
    parser.add_argument(
...
def worker(rank, dataset_chunk, output_queue, args, device):
    ...
    if args.sp_mode == "lsh":
        ...
        cluster_centers = torch.load(args.cc_path, weights_only=True)
