...
def kmeans_reject_overlap(
    ...
    Returns:
        ...
        score (float): separation between closest and second closest centroid
    """

    # Encode text
    emb = embedder.encode(text, convert_to_tensor=True).reshape(1, -1)

    # Ensure torch tensor on correct device
    if not torch.is_tensor(cluster_centers):
        ...
        cluster_centers = cluster_centers.to(emb.device)

    # Cosine distances
    dis = pairwise_cosine(emb, cluster_centers, device=emb.device)

    # Sort distances (lower = closer)
    ranked = torch.argsort(dis)   # shape: (K,)

    closest = ranked[0]
    second = ranked[1]

    first_dist = dis[closest]
    second_dist = dis[second]


    score = (second_dist - first_dist).item()
...
def kmeans_reject_completion(
    ...
    while True:
        ...
        accepted_text, curr_cluster_id = kmeans_reject_overlap(text=new_text, embedder=embedder, cluster_centers=cluster_centers, margin=0.01)
